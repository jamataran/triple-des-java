...
public class Base64 {
    ...
    private String encode(byte[] bin) {
     ...
     */
    private String encode(byte[] bin, int str, int len) {
        int ol;
        StringBuffer sb;
        int lp;
        int el;
        ...
        if (ll == 1) {
            sb.append(encode(new byte[]{bin[el + str], 0, 0}).substring(0, 2)).append("==");
        } else if (ll == 2) {
            sb.append(encode(new byte[]{bin[el + str], bin[el + str + 1], 0}).substring(0, 3)).append("=");
        }
        if (ol != sb.length()) {
